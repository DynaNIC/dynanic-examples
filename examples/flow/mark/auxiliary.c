/* auxiliary.c: Common functions used in the MARK example.
...
void print_simple_stats(struct mark_stats *m_stats) {
    uint64_t bytes = 0;
    uint64_t packets = 0;
    uint16_t portid = 0;
    ...
    printf("\nPackets marked ONE: %" PRIu64 "\n", m_stats->one);
    printf("Packets marked TWO: %" PRIu64 "\n", m_stats->two);
    printf("Packets with other mark: %" PRIu64 "\n", m_stats->other);
